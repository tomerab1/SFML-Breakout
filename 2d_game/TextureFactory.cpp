...
std::unordered_map<std::string, std::shared_ptr<sf::Texture>> TextureFactory::m_textureMap;

void TextureFactory::loadTexture(std::string&& texturePath,
                                 std::string&& textureName) {
  sf::Texture texture;

  if (!texture.loadFromFile(texturePath)) throw new std::runtime_error("Could not load texture from " + texturePath);
